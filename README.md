...
EfficientNet、BiFPN、ASFF、DCN

## 概述

PaddlePaddle复现YOLOv3！使用DIOU loss训练。
请查看
...
这个仓库有很大一部分参考（用paddle的api翻译）了YunYang1994的代码（label的填写以及损失函数部分），这里致敬大佬！
大部分为自己原创（我只是个搬砖的）。
...
1_pytorch2paddle.py  将pytorch模型导出为paddle模型。
demo.py             用paddle模型进行预测。
eval.py             对paddle模型评估。跑完这个脚本后需要再跑mAP/main.py进行mAP的计算。
...
## 评估
训练完成后，运行eval.py对paddlepaddle模型评估，跑完这个脚本后需要再跑mAP/main.py进行mAP的计算。
