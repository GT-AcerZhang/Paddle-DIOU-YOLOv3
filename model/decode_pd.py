...
class Decode(object):
    def __init__(self, inputs, obj_threshold, nms_threshold, input_shape, model_path, file_path, initial_filters, use_gpu):
        self._t1 = obj_threshold
        self._t2 = nms_threshold
        self.input_shape = input_shape
        self.all_classes = self.get_classes(file_path)
        self.num_classes = len(self.all_classes)
        self.y1, self.y2, self.y3 = YOLOv3(inputs, initial_filters, self.num_classes, True)
    ...
    def predict(self, image, shape):
        ...
        outs = self.exe.run(fluid.default_main_program(),
                        feed={'input_1': image, },
                        fetch_list=[self.y1, self.y2, self.y3])
